 ...
 * modified by Michail Pishchagin <mailfrom@mail.ru>
 */
...
{
   struct file *p, *f = (struct file *)malloc (sizeof *f);
...
static void table_add (DATAFILE *d, AL_CONST char *prefix)
{
    ...
    int i, n;
/*
    ustrcpy (path, strlwr ((char *)prefix) );
    n = data_append (d);
    if (n)
...
*/
    ...
    {
	      name = strlwr ( (char *) get_datafile_property (&d[i], DAT_NAME) );
...

         {
	      ...
	      if (d[i].type == DAT_FILE) {
	         ...
	         table_add ((DATAFILE *)d[i].dat, path);
	      }
    }
}
...
{
    int n = (int) hash_lookup (/*strlwr(*/key/*)*/, &table);
    return n;
}
