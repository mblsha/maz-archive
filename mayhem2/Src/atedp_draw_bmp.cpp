...
void draw_vp_back_bmp (VIEWPORT *vp, BITMAP *bmp)
{
   int x,  y, x1, x2, y1, y2;
   int xx, yy;
   int tx1, ty1, tx2, ty2;
...
//   int clip;
   ...
   for(x = (tx2 - tx1); x >= 0; x--)
   {
      if((tile  = tile_map.back_plane[loc])  > 0)
         draw_sprite(bmp, back_tile_bmp[tile],  xx, y1);

      if((tile2 = tile_map.back_plane[loc2]) > 0)
         draw_sprite(bmp, back_tile_bmp[tile2], xx, y2);
   ...
   {
      if((tile  = tile_map.back_plane[loc])  > 0)
         draw_sprite(bmp, back_tile_bmp[tile],  x1, yy);

      if((tile2 = tile_map.back_plane[loc2]) > 0)
         draw_sprite(bmp, back_tile_bmp[tile2], x2, yy);
   ...
   for(y = (ty2 - ty1); y > 0; y--)
   {
      ...
      {
         if((tile = tile_map.back_plane[loc]) > 0)
            draw_sprite(bmp, back_tile_bmp[tile], xx, yy);
...
void draw_vp_planes_bmp (VIEWPORT *vp, BITMAP *bmp)
{
   int x,  y, x1, x2, y1, y2;
   int xx, yy;
   int tx1, ty1, tx2, ty2;
   ...
   for(x = (tx2 - tx1); x >= 0; x--)
   {
      ...
      {
         if((tile  = tile_map.back_plane[loc])  > 0)
            draw_sprite(back, back_tile_bmp[tile],  xx, y1);

         if((tile2 = tile_map.back_plane[loc2]) > 0)
            draw_sprite(back, back_tile_bmp[tile2], xx, y2);
      }
      if(vp->fore)
      {
         if((tile  = tile_map.fore_plane[loc])  > 0)
            draw_sprite(fore, fore_tile_bmp[tile],  xx, y1);

         if((tile2 = tile_map.fore_plane[loc2]) > 0)
            draw_sprite(fore, fore_tile_bmp[tile2], xx, y2);
      }
   ...
   for(y = (ty2 - ty1); y >= 0; y--)
   {
      ...
      {
         if((tile  = tile_map.back_plane[loc])  > 0)
            draw_sprite(back, back_tile_bmp[tile],  x1, yy);

         if((tile2 = tile_map.back_plane[loc2]) > 0)
            draw_sprite(back, back_tile_bmp[tile2], x2, yy);
      }
      if(vp->fore)
      {
         if((tile  = tile_map.fore_plane[loc])  > 0)
            draw_sprite(fore, fore_tile_bmp[tile],  x1, yy);

         if((tile2 = tile_map.fore_plane[loc2]) > 0)
            draw_sprite(fore, fore_tile_bmp[tile2], x2, yy);
      }
   ...
   for(y = (ty2 - ty1); y > 0; y--)
   {
      ...
      for(x = (tx2 - tx1); x > 0; x--)
      {
         ...
         {
            if((tile = tile_map.back_plane[loc]) > 0)
               draw_sprite(back, back_tile_bmp[tile], xx, yy);
         }
         if(vp->fore)
         {
            if((tile = tile_map.fore_plane[loc]) > 0)
               draw_sprite(fore, fore_tile_bmp[tile], xx, yy);
         }
         xx += tile_map.tile_size;
         loc++;
      }
...
void draw_vp_fore_bmp (VIEWPORT *vp, BITMAP *bmp)
{
   int x,  y, x1, x2, y1, y2;
   int xx, yy;
   int tx1, ty1, tx2, ty2;
   ...
   {
      if((tile  = tile_map.fore_plane[loc])  > 0)
         draw_sprite(bmp, fore_tile_bmp[tile],  xx, y1);

      if((tile2 = tile_map.fore_plane[loc2]) > 0)
         draw_sprite(bmp, fore_tile_bmp[tile2], xx, y2);
   ...
   {
      if((tile  = tile_map.fore_plane[loc])  > 0)
         draw_sprite(bmp, fore_tile_bmp[tile],  x1, yy);

      if((tile2 = tile_map.fore_plane[loc2]) > 0)
         draw_sprite(bmp, fore_tile_bmp[tile2], x2, yy);
   ...
   for(y = (ty2 - ty1); y > 0; y--)
   {
      ...
      {
         if((tile = tile_map.fore_plane[loc]) > 0)
            draw_sprite(bmp, fore_tile_bmp[tile], xx, yy);
...
void draw_vp_info_bmp (VIEWPORT *vp, BITMAP *bmp)
{
   int x,  y, x1, x2, y1, y2;
   int xx, yy;
   int tx1, ty1, tx2, ty2;
   ...
   for(x = (tx2 - tx1); x >= 0; x--)
   {
      if(((tile  = tile_map.info_plane[loc])  > 0) && (tile  < info_tile_gfx_count))
         draw_sprite(bmp, info_tile_bmp[tile],  xx, y1);

      if(((tile2 = tile_map.info_plane[loc2]) > 0) && (tile2 < info_tile_gfx_count))
         draw_sprite(bmp, info_tile_bmp[tile2], xx, y2);
   ...
   {
      if(((tile  = tile_map.info_plane[loc])  > 0) && (tile  < info_tile_gfx_count))
         draw_sprite(bmp, info_tile_bmp[tile],  x1, yy);

      if(((tile2 = tile_map.info_plane[loc2]) > 0) && (tile2 < info_tile_gfx_count))
         draw_sprite(bmp, info_tile_bmp[tile2], x2, yy);
   ...
   for(y = (ty2 - ty1); y > 0; y--)
   {
      ...
      {
         if(((tile = tile_map.info_plane[loc]) > 0) && (tile < info_tile_gfx_count))
            draw_sprite(bmp, info_tile_bmp[tile], xx, yy);
